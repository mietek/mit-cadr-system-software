  ...
  (SELECTQ OP
    ...
    ((:PRINT :PRINT-SELF)
	    (FORMAT (CAR ARGS) "#<Channel ~S ~O>"
                    (CHANNEL-FILE-NAME CHANNEL) (%POINTER CHANNEL)))
...
(DEFSELECT (FILE-CHAOSNET-READ-STREAM FILE-CHANNEL-OPERATIONS)
  ...
  (:CLOSE . FILE-CLOSE)
  (:NAME () (CHANNEL-FILE-NAME FILE-CHANNEL))
...
(DEFSELECT (FILE-CHANNEL-OPERATIONS FILE-STREAM-DEFAULT-HANDLER)
  ...
  (:RENAME (NEW-NAME &AUX SUCCESS STRING)
    (SELECTQ (CHANNEL-STATE FILE-CHANNEL)
      ((:OPEN :EOF :SYNC-MARKED :ASYNC-MARKED)
       (SETQ NEW-NAME (FILE-EXPAND-PATHNAME NEW-NAME))	;Canonicalize filename
       (MULTIPLE-VALUE (STRING SUCCESS)
         (FILE-CHANNEL-OPERATIONS ':COMMAND NIL (FORMAT NIL "RENAME~%~A~%" NEW-NAME)))
       (COND (SUCCESS
...
(DEFUN FILE-STREAM-DEFAULT-HANDLER (OP &OPTIONAL ARG1 &REST ARGS)
  (STREAM-DEFAULT-HANDLER (SELECTQ (CHANNEL-DIRECTION FILE-CHANNEL)
			  ...
			  OP ARG1 ARGS))

(DEFUN FILE-OP (OP FILENAME &REST ARGS &AUX DEVICE HANDLER)
  (MULTIPLE-VALUE (FILENAME DEVICE)
    (FILE-EXPAND-PATHNAME FILENAME))
  (OR DEVICE
      (SETQ DEVICE (NSUBSTRING FILENAME 0 (STRING-SEARCH-CHAR #/: FILENAME))))
  (SETQ HANDLER (ASSOC DEVICE FILE-DEVICES))	;Get handler for this device
  (OR HANDLER			    		;If device unknown, use the default device
      (SETQ HANDLER (ASSOC (SETQ DEVICE FILE-DEFAULT-HOST) FILE-DEVICES)))
  (OR HANDLER (FERROR NIL "Default device (~A) is not a known device." FILE-DEFAULT-HOST))
  (LEXPR-FUNCALL (CDR HANDLER) OP DEVICE FILENAME ARGS))

(DEFSELECT FILE-CHAOS-OP-DISPATCH
  (:OPEN . OPEN-CHAOS)
  (:RENAME . RENAME-CHAOS)
  (:DELETE . DELETE-CHAOS))
...
(DEFUN OPEN (FILENAME &OPTIONAL OPTIONS EXCEPTION-HANDLER)
  ...
  (FILE-OP ':OPEN FILENAME OPTIONS EXCEPTION-HANDLER))
  
(DEFUN OPEN-CHAOS (IGNORE DEVICE FILENAME OPTIONS EXCEPTION-HANDLER
  ...
  (DO-NAMED OPEN-CHAOS () (NIL)	;DO repeated if retrying from error
	...
	(SELECTQ (CAR L)
      ...
      (SETF (CHANNEL-STATE FILE-CHANNEL)
	    (SELECTQ MODE
	      ((:WRITE :READ) ':OPEN)
	      (:PROBE ':CLOSED)
	      (OTHERWISE (FERROR NIL "Mode ~S is unknown.  This is an impossible error" MODE))))
	...
	(FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':COMMAND NIL (NEQ MODE ':PROBE) NIL
		 "OPEN " MODE " " TYPE
		 (FORMAT NIL "~:[~;~0G BYTE-SIZE ~D~]~%~A~%" BYTE-SIZE FILENAME)))
      (COND ((NOT SUCCESS)
	     (SETQ STRING (STRING-APPEND STRING))
	     (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':RETURN PKT)
	     (COND (NOERROR-P
...
(DEFUN RENAMEF (STRING-OR-STREAM NEW-NAME &OPTIONAL (ERROR-P T))
  (SETQ NEW-NAME (FILE-EXPAND-PATHNAME NEW-NAME))
  (COND ((STRINGP STRING-OR-STREAM)
	 (FILE-OP ':RENAME STRING-OR-STREAM NEW-NAME ERROR-P))
	(T (FUNCALL STRING-OR-STREAM ':RENAME NEW-NAME))))

(DEFUN RENAME-CHAOS (IGNORE OLD-HOST FILENAME NEW-NAME ERROR-P)
  (LET ((PKT) (SUCCESS) (STRING) (FILE-CHANNEL)
	      (NEW-HOST (NSUBSTRING NEW-NAME 0 (STRING-SEARCH-CHAR #/: NEW-NAME))))
    (COND ((STRING-EQUAL NEW-HOST OLD-HOST)
	   (SETQ FILE-CHANNEL (CHANNEL-ALLOCATE OLD-HOST NIL NIL))
	   (MULTIPLE-VALUE (PKT SUCCESS STRING)
	      (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':COMMAND NIL NIL NIL
		       (FORMAT NIL "RENAME~%~A~%~A~%" FILENAME NEW-NAME)))
	   (COND (SUCCESS
		  (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':RETURN PKT)
		  T)
		 ((NOT ERROR-P)
		 ...
		 (T (UNWIND-PROTECT (FILE-PROCESS-ERROR STRING FILENAME T)
		      (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':RETURN PKT))
		    ;; Retry if proceeded
		    (RENAME-CHAOS NIL OLD-HOST FILENAME NEW-NAME ERROR-P))))
	  (T (FERROR NIL "Renames across hosts not permitted.")))))

(DEFUN DELETEF (STRING-OR-STREAM &OPTIONAL (ERROR-P T))
  (COND ((STRINGP STRING-OR-STREAM)
	 (FILE-OP ':DELETE STRING-OR-STREAM ERROR-P))
	(T (FUNCALL STRING-OR-STREAM ':DELETE))))

(DEFUN DELETE-CHAOS (IGNORE HOST FILENAME ERROR-P)
  (LET ((PKT) (SUCCESS) (STRING) (FILE-CHANNEL))
    (SETQ FILE-CHANNEL (CHANNEL-ALLOCATE HOST NIL NIL))
    (MULTIPLE-VALUE (PKT SUCCESS STRING)
       (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':COMMAND NIL NIL NIL
		(FORMAT NIL "DELETE~%~A~%" FILENAME)))
    (COND (SUCCESS
	   (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':RETURN PKT)
	   T)
	  ((NOT ERROR-P)
	  ...
	  (T (UNWIND-PROTECT (FILE-PROCESS-ERROR STRING FILENAME T)
	       (FUNCALL (CHANNEL-FUNCTION FILE-CHANNEL) ':RETURN PKT))
	     (DELETE-CHAOS NIL HOST FILENAME ERROR-P)))))	;retry if proceeded
...
(DEFUN FILE-PROCESS-ERROR (STRING STR-OR-CHAN PROCEEDABLE
  ...
  (PROG ()
    (COND ((EQ (TYPEP STR-OR-CHAN) 'CHANNEL)
	   ...
	   (SETQ WHO-FOR (CHANNEL-FILE-NAME STR-OR-CHAN)))
...
(DEFUN FILE-LOGIN (USER-ID)
  (DOLIST (ALIST-ENTRY FILE-HOST-ALIST)
    (HOST-UNIT-MAP-FUNCTION (INIT-FORM ALIST-ENTRY)
			    #'(LAMBDA (U ID)
				      (FUNCALL (HOST-UNIT-FUNCTION U) ':LOGIN-UNIT U ID))
			    USER-ID)))
...
(DEFUN FILE-USER-ID-HSNAME (&OPTIONAL (HOST USER-LOGIN-MACHINE) RESET-P
			    &AUX HOST-ITS UNIT)
  (FORCE-USER-TO-LOGIN)
  (SETQ HOST-ITS (INIT-FORM (OR (ASSOC HOST FILE-HOST-ALIST)
  ...
  (OR (HOST-UNIT-GRAB UNIT (FUNCALL HOST-ITS ':VALIDATE-CONTROL-CONNECTION UNIT))
      (FERROR NIL "Cannot connect to ~A" HOST))
  (STRING-APPEND HOST ": " (CDR (ASSOC HOST USER-HSNAMES)) "; "))

(DEFSELECT HOST-ITS
  ...
  (:RESET ()
	...
	((NULL UNIT)
	 (SETF (HOST-UNIT-LINK FILE-HOST-FIRST-UNIT) NIL))
      (HOST-ITS ':RESET-UNIT UNIT NIL)))	;Arg of NIL: Unlock all unit
  ...
  (:LOGIN-UNIT (UNIT USER-ID &AUX (CONN (HOST-UNIT-CONTROL-CONNECTION UNIT)))
	   ...
	   (AND USER-ID (NOT (EQUAL USER-ID ""))
		(LET ((STR (CHAOS:PKT-STRING PKT))
		  ...
		  (LET ((HSNAME (SUBSTRING STR (SETQ IDX (1+ IDX))
			...
			(RPLACD ITEM HSNAME)
			(PUSH (CONS (HOST-UNIT-HOST UNIT) HSNAME) USER-HSNAMES)))
  ...
  (:VALIDATE-CONTROL-CONNECTION (UNIT &AUX (CONN (HOST-UNIT-CONTROL-CONNECTION UNIT)))
    (COND ((AND CONN
		...
		(NOT (DOLIST (DATA-CONN (HOST-UNIT-DATA-CONNECTIONS UNIT))
			   ...
			   (RETURN T)))))
	   T)
	  (T (HOST-ITS ':RESET-UNIT UNIT T)	;Arg of T means don't unlock lock
		   ...
		   (T (SETF (HOST-UNIT-CHANNEL-FUNCTION UNIT) 'FILE-CHAOSNET-CHANNEL-FUNCTION)
		      (SETF (CHAOS:INTERRUPT-FUNCTION (HOST-UNIT-CONTROL-CONNECTION UNIT))
			    (LET ((FILE-HOST-UNIT UNIT))
			      (CLOSURE '(FILE-HOST-UNIT) 'HOST-ITS-INTERRUPT-FUNCTION)))
		      (HOST-ITS ':LOGIN-UNIT UNIT USER-ID)
		      T)))))
...
(DEFUN HOST-ITS-INTERRUPT-FUNCTION (REASON CONN &REST IGNORE)
  (SELECTQ REASON
    (:INPUT
  ...
  (PROG ((DIRECTION (COND (WRITE-P ':OUTPUT)
    ...
    (COND ((NOT DATA-CONN-P)
	   (OR (HOST-UNIT-GRAB FILE-HOST-FIRST-UNIT
	       ...
	       (FERROR NIL "Cannot connect to host ~S"
    ...
    (DO-NAMED HAVE-DATA-CONN
		 ...
		 (SETF (HOST-UNIT-CLOSURE SELECTED-UNIT)
	       ...
	       (HOST-UNIT-UNLOCK PREV-UNIT))
      (AND PREV-UNIT (HOST-UNIT-UNLOCK PREV-UNIT))
      (HOST-UNIT-LOCK UNIT)
    ...
    (COND ((NULL SELECTED-UNIT) (FERROR NIL "No unit selected"))
          (SELECTED-DATA-CONN)
          (T (UNWIND-PROTECT-IF-ABNORMAL-EXIT
	      (PROGN (OR (FUNCALL (HOST-UNIT-FUNCTION SELECTED-UNIT)
	      ...
	      (HOST-UNIT-UNLOCK SELECTED-UNIT))))
...
(DEFUN WHO-LINE-FILE-STATE (ITEM &AUX (MAX-CHARS 36.) IDLE)
  (COND (FILE-CHANNEL-CURRENT
	 (LET ((PERCENT 0)
	       ...
	       (CURRENT) (STRING) (SP-POS) (FILE-NAME))
	   ...
	   (AND LENGTH (NOT (ZEROP LENGTH))
		(SETQ PERCENT (// (* 100. CURRENT)
				  LENGTH)))
	   TV:(SHEET-SET-CURSORPOS WHO-LINE-WINDOW (WHO-LINE-ITEM-LEFT SI:ITEM) 0)
	   (COND ((AND (EQ OLD-CHANNEL FILE-CHANNEL-CURRENT)
		 ...
		 (T (OR (EQ OLD-CHANNEL FILE-CHANNEL-CURRENT)
		    ...
		    TV:(SHEET-STRING-OUT WHO-LINE-WINDOW "  ")
		    (SETQ SP-POS (+ 4 (STRING-LENGTH FILE-NAME)))
		    TV:(SHEET-CLEAR-EOL WHO-LINE-WINDOW)
...
(ADD-INITIALIZATION "FILE-COMPUTER:AI"
  '(PROGN
     (ADD-INITIALIZATION "FILE-COMPUTER:AI"
			 `(FUNCALL ',(FILE-HOST "AI" 'HOST-ITS) ':RESET) '(SYSTEM))
     (SETQ FILE-DEVICES (CONS '("AI" . FILE-CHAOS-OP-DISPATCH) FILE-DEVICES)))
  '(ONCE))

(ADD-INITIALIZATION "FILE-COMPUTER:MC"
 '(PROGN
    (ADD-INITIALIZATION "FILE-COMPUTER:MC"
			`(FUNCALL ',(FILE-HOST "MC" 'HOST-ITS) ':RESET) '(SYSTEM))
    (SETQ FILE-DEVICES (CONS '("MC" . FILE-CHAOS-OP-DISPATCH) FILE-DEVICES)))
 '(ONCE))
