...
ARRAY-TYPE-REF-DISPATCH
	...
	(QQARY)		;SPEC-PDL
	(INHIBIT-XCT-NEXT-BIT TRAP)	;TV BUFFER (OBSOLETE)
	(QQARY)		;REG-PDL
	(QFARY)		;FLOAT
...
SCONS	(CALL-LESS-OR-EQUAL M-B A-ZERO TRAP)
...
SCONSR	(CALL CONS-GET-AREA)			;Set up M-S
    (ERROR-TABLE ARGTYP AREA M-S NIL)
...
LCONS	(CALL-LESS-OR-EQUAL M-B A-ZERO TRAP)
		(ERROR-TABLE CONS-ZERO-SIZE M-B)
...
LCONSR	(CALL CONS-GET-AREA)			;Set up M-S
    (ERROR-TABLE ARGTYP AREA M-S NIL)
	((VMA-START-READ) ADD M-S A-V-AREA-REGION-LIST)	;Find appropriate region of the area
...
D-CONS-1	;DISPATCH ON SPACE TYPE
	...
	(INHIBIT-XCT-NEXT-BIT CONSF5)		;6 EXIT (TRY NEXT REGION)
...
XAAIXL (MISC-INST-ENTRY ARRAY-ACTIVE-LENGTH)
   ...
   (ERROR-TABLE CALLS-SUB ARRAY-ACTIVE-LENGTH)
	...
	(JUMP-NOT-EQUAL M-TEM (A-CONSTANT (EVAL DTP-FIX)) XAIXL2) ;Yes, it's the fill ptr
...
XSTRNP	((M-A) C-PDL-BUFFER-POINTER-POP)
	...
	(JUMP-NOT-EQUAL M-ZR (A-CONSTANT (EVAL (LSH ART-STRING ARRAY-TYPE-SHIFT))) XFALSE)
	(JUMP XTRUE)
