...
(DEFUN FED (&OPTIONAL (SCREEN TV-DEFAULT-SCREEN) (WIDTH 700) (HEIGHT 600) &AUX FRAME)
    (OR FED-WINDOW
...
(DEFUN FED-COMPLEMENT-SQUARE (&OPTIONAL X Y &AUX OLD-INTEN)
    (LOCAL-DECLARE ((SPECIAL LEFT-MARGIN TOP-MARGIN CHARACTER-ARRAY
      ...
      (SETQ OLD-INTEN (PLANE-AR-N CHARACTER-ARRAY X Y))
      (PLANE-AS-N (COND ((ZEROP OLD-INTEN) 1) (T 0))
                  CHARACTER-ARRAY X Y)
...
;Complement the square under the mouse, and then as long as you hold the button down
;whenever the mouse is moved to a new square that square is complemented also.
(DEFUN FED-MOUSE-COMPLEMENT-SQUARES (&AUX X Y OLD-M-X OLD-M-Y)
    (LOCAL-DECLARE ((SPECIAL LEFT-MARGIN TOP-MARGIN BOX-X-SIZE BOX-Y-SIZE
      ...
      (DO ((FIRST T NIL))
        ...
        (OR (AND (NOT FIRST)
            ...
            (PROGN (FED-COMPLEMENT-SQUARE X Y)
                   (<- SELF ':UPDATE)))
