...
(defun ctest-move (probe dir dist spd
 ...
 (cond ((null ctest-use-ramping)
  ...
  (prog (ramp-steps cruise-steps code rate)
	...
	(setq ramp-steps (min (// (- base-speed spd) ramp-rate)
			      (// dist 2)))
...
(DEFMETHOD (PROBE-CLASS :APPARENT-ERROR) (XE YE &AUX TEM)
  (SETQ XCE (// (+ (* 9 XCE) (* XE 100.)) 10.))
  (SETQ YCE (// (+ (* 9 YCE) (* YE 100.)) 10.))
  (COND ((> XCE 70.)
	 (SETQ TEM (MAX 1 (// XCE 100.)))
	 (SETQ XPOS-STEPS (- XPOS-STEPS TEM))
	 (SETQ XCE 0)
	 (FORMAT T "~%CORRECTING ~S XPOS-STEPS -~S" NAME TEM))
	((< XCE -70.)
	 (SETQ TEM (MAX 1 (// (MINUS XCE) 100.)))
	 (SETQ XPOS-STEPS (+ XPOS-STEPS TEM))
	 (SETQ XCE 0)
	 (FORMAT T "~%CORRECTING ~S XPOS-STEPS +~S" NAME TEM)))
  (COND ((> YCE 70.)
	 (SETQ TEM (MAX 1 (// YCE 100.)))
	 (SETQ YPOS-STEPS (- YPOS-STEPS TEM))
	 (SETQ YCE 0)
	 (FORMAT T "~%CORRECTING ~S YPOS-STEPS -~S" NAME TEM))
	((< YCE -70.)
	 (SETQ TEM (MAX 1 (// (MINUS YCE) 100.)))
	 (SETQ YPOS-STEPS (+ YPOS-STEPS TEM))
	 (SETQ YCE 0)
	 (FORMAT T "~%CORRECTING ~S YPOS-STEPS +~S" NAME TEM))))
...
(DEFMETHOD (PROBE-CLASS :CALIBRATE) ()
  (PROG (XD YD REDO-CALIB NBOARDS POS)
	(FORMAT T "~%REDO CALIBRATOR?")
	(COND ((SETQ REDO-CALIB (Y-OR-N-P))
