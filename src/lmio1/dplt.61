...
(DEFUN OUTPUT-FILE (FILE)
  (UNWIND-PROTECT 
    (PROGN
      (SETQ STREAM (OPEN (SI:FILE-DEFAULT-FN2 FILE "PLT") '(:IN :FIXNUM :BYTE-SIZE 9.))
...
(DEFUN DRAW-FRAME-AND-TITLE (&AUX
     ...
     (DATE-TIME   (FORMAT NIL "~D-~[JAN~;FEB~;MAR~;APR~;MAY~;JUN~;JUL~;AUG~;SEP~;OCT~;NOV~;DEC~]-~D  ~2,'0D:~2,'0D"
			  ...
			  (// BINARY-TIME 60.)
			  (\ BINARY-TIME 60.)))
     (FILE-NAME   (FORMAT NIL "~A:~A; ~A"
			  (FIRST (SI:FILE-SPREAD-PATHNAME (FUNCALL STREAM ':NAME)))
			  (SIXBIT-TO-STRING SIXBIT-DIR) (SIXBIT-TO-STRING SIXBIT-FN1)))
     (LEFT   (FLEFT FRAME))
     (RIGHT  (FRIGHT FRAME))
     (TOP    (FTOP FRAME))
