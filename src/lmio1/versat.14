...
(DEFUN VERSATEC-QWOPY (&OPTIONAL (SCREEN SI:TV-CPT-SCREEN)
  ...
  (SETQ ARRAY (MAKE-ARRAY NIL 'ART-8B
			  (LIST (SETQ W (// (SCREEN-WIDTH SCREEN) 8.))
...
(DEFUN VERSATEC-COPY (&OPTIONAL (INPUT-ARRAY (SCREEN-BUFFER-PIXEL-ARRAY SI:TV-CPT-SCREEN))
  ...
  (VERSATEC-RESET)
  (SETQ LINE-ARRAY (MAKE-ARRAY NIL 'ART-1B (* 2 H)))
  (SETQ ARRAY-8B (MAKE-ARRAY NIL 'ART-8B (SETQ A8W (// H 4)) LINE-ARRAY))
  (UNWIND-PROTECT
    (PROGN
...
(DEFUN VERSATEC-BIG-COPY (&OPTIONAL (INPUT-ARRAY (SCREEN-BUFFER-PIXEL-ARRAY SI:TV-CPT-SCREEN))
			  ...
			  &AUX ARRAY-8B LINE-ARRAY
			       ...
			       (VERSATEC-CONTROL VERSATEC-PLOTTER-CONTROL))
  (VERSATEC-RESET)
  (SETQ LINE-ARRAY (MAKE-ARRAY NIL 'ART-1B LSIZE))
  (SETQ ARRAY-8B (MAKE-ARRAY NIL 'ART-8B (SETQ A8W (// LSIZE 8)) LINE-ARRAY))
  (UNWIND-PROTECT
    (PROGN
      (SETQ UB-ADR (WIRE-ARRAY-AND-MAP LINE-ARRAY T 766142))
      (DOTIMES (Y H)
...
(DEFUN WIRE-ARRAY-AND-MAP (ARRAY &OPTIONAL (WIRE-P T) (UB-MAP-LOC 766140))
  (LET ((NPAGES (1+ (- (// (+ (%POINTER ARRAY) (%STRUCTURE-TOTAL-SIZE ARRAY))
			   SYS:PAGE-SIZE)
		       (// (%POINTER ARRAY) SYS:PAGE-SIZE))))
