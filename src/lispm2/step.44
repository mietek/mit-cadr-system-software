...
(DEFUN TERPRI-IF-INSUFFICIENT-SPACE (PERCENT-WIDTH &AUX INSIDE-WIDTH)
  (IF (MEMQ ':INSIDE-SIZE (FUNCALL STANDARD-OUTPUT ':WHICH-OPERATIONS))
      (SETQ INSIDE-WIDTH (FUNCALL STANDARD-OUTPUT ':INSIDE-SIZE))
      (LET ((PC-PPR (FUNCALL STANDARD-OUTPUT ':PC-PPR)))
	(SETQ INSIDE-WIDTH (- (PC-PPR-RIGHT-MARGIN PC-PPR)
			      (PC-PPR-LEFT-MARGIN PC-PPR)))))
  (LET ((X (// (* PERCENT-WIDTH INSIDE-WIDTH) 100.)))
    (AND ( (FUNCALL STANDARD-OUTPUT ':READ-CURSORPOS ':PIXEL) X)
	 (TERPRI))))

(DEFUN PRINT-TRUNCATED (SEXP PERCENT-WIDTH &AUX INSIDE-WIDTH)
  (IF (MEMQ ':INSIDE-SIZE (FUNCALL STANDARD-OUTPUT ':WHICH-OPERATIONS))
      (SETQ INSIDE-WIDTH (FUNCALL STANDARD-OUTPUT ':INSIDE-SIZE))
      (LET ((PC-PPR (FUNCALL STANDARD-OUTPUT ':PC-PPR)))
	(SETQ INSIDE-WIDTH (- (PC-PPR-RIGHT-MARGIN PC-PPR)
			      (PC-PPR-LEFT-MARGIN PC-PPR)))))
  (LET ((PRINT-TRUNCATED (// (* PERCENT-WIDTH INSIDE-WIDTH) 100.)))
