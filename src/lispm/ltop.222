...
(DEFUN LISP-REINITIALIZE (&OPTIONAL (CALLED-BY-USER T))
  ...
  (COND ((FBOUNDP 'TV:WINDOW-INITIALIZE)
         (<- CURRENT-PROCESS ':RUN-REASON 'LISP-INITIALIZE)))
  ...
  (COND ((FBOUNDP 'TV:WINDOW-INITIALIZE)
	 ...
	 (<- CURRENT-PROCESS ':REVOKE-RUN-REASON 'LISP-INITIALIZE))
...
(DEFUN BREAK (&QUOTE TAG &OPTIONAL &EVAL (CONDITIONAL T)
  ...
  (COND (CONDITIONAL
	 ...
	 (AND (NOT (NULL CURRENT-PROCESS))
	      (NULL (<- CURRENT-PROCESS ':RUN-REASONS))
	      (<- CURRENT-PROCESS ':RUN-REASON 'BREAK))
...
(DEFUN PROCESS-WAIT (WHOSTATE FUNCTION &REST ARGUMENTS)
  (COND ((APPLY FUNCTION ARGUMENTS)	;Test condition before doing slow stack-group switch
	...
	(T
	 ...
	 (TV:WHO-LINE-PROCESS-CHANGE CURRENT-PROCESS)	;Show as waiting in wholine
	 ...
	 (TV:WHO-LINE-PROCESS-CHANGE CURRENT-PROCESS)))) ;Show as running in wholine
