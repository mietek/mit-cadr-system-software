...
(DEFUN FASLOAD (FILE-NAME &OPTIONAL PKG
      ...
      ;;Default the FN2 to QFASL and fill in defaults
...
(DEFUN FASL-APPEND (OUTFILE &REST INFILES &AUX FASD-STREAM)
    (SETQ FASD-STREAM (OPEN (FILE-DEFAULT-FN2 OUTFILE "QFASL")
			    '(WRITE FIXNUM)))
    (COMPILER:FASD-START-FILE)
    (MAPC (FUNCTION (LAMBDA (INFILE)
	      (SETQ INFILE (FILE-DEFAULT-FN2 INFILE "QFASL"))
	      (FILE-QFASL-P INFILE)
	      (LET ((ISTREAM (OPEN INFILE '(READ FIXNUM))))
                   (FUNCALL ISTREAM ':TYI)
                   (FUNCALL ISTREAM ':TYI)
