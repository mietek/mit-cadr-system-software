;;; -*- Mode: Lisp; Package: TV; Lowercase: T; Base: 8 -*-
...
(defun clipped-line (x0 y0 x1 y1 xl yl xh yh &optional (sheet selected-window) &aux clipped-p)
  (if (< x0 xl) (setq x0 xl
		      clipped-p t))
  (if (< x1 xl) (setq x1 xl
		      clipped-p t))
  ...
  (if (> y1 yh) (setq y1 yh
		      clipped-p t))
  (funcall sheet ':draw-line x0 y0 x1 y1 alu-xor)
  clipped-p)

(defun crack (p x0 y0 x1 y1 xl yl xh yh &optional (sheet selected-window) &aux x y)
  (if (> (frandom) p)
...
(defun jag-ray (p pl x0 y0 dx dy xl yl xh yh &optional (screen selected-window) last-ray
		  &aux x1 y1 this-ray)
  (do ()
      ((clipped-line x0 y0
...
(defun outspoken (x0 y0 xl yl xh yh &optional (sheet selected-window)
...
(defun craze (&optional (sheet selected-window)
			(xl (sheet-inside-left sheet))
			(yl (sheet-inside-top sheet))
			(xh (sheet-inside-width sheet))
			(yh (sheet-inside-height sheet)))
